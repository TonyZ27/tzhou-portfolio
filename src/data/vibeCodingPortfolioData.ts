export const vibeCodingPortfolioData = {
  ...
  whyAbandon: {
    ...
    content: {
      en: '<p>When I started building my personal site, my first instinct was to use no-code tools like Framer or Figma Site. Their advantage is obvious: they let me stay within my comfort zone and quickly turn familiar design workflows into live frontend pages. But the problem is that the moment you touch specific interaction details, complex responsive logic, or custom web animations, the limitations of these tools become glaringly apparent.</p><p>As a UX designer, I still want pixel-level control over the final web presentation, but I don\'t want to burn massive amounts of time hand-coding frontend tags from scratch. The rise of Vibe Coding offered a way out. It gave me access to React and underlying code logic, while utilizing AI to drastically compress the learning curve of manual coding.</p>',
      zh: '<p>在搭建个人网站时，我一开始尝试使用 Framer 或 Figma Site 这类无代码工具。它们的优势很明显：能在我的舒适区内，快速把我熟悉的设计工作流转化为可上线的前端页面。但问题在于，一旦涉及特定的交互细节、复杂的自适应逻辑或定制化的网页动画，这些工具的局限性就会凸显出来。</p><p>作为一名 UX 设计师，我仍希望对最终的网页呈现有像素级的把控力，但我并不想将大量时间耗费在从零手写前端标签上。Vibe Coding 的兴起为解决这个问题提供了一种思路。它让我能够触及 React 和更底层的代码逻辑，同时利用 AI 大幅压缩了手搓代码的学习成本。</p>',
    },
  },
  ...
  workflow: {
    ...
    content: {
      en: "<p>Naturally, my workflow starts with page design. Even though I sometimes feel my own aesthetic sense might not beat the output of certain AI visual skills, getting hands-on during the design phase allows me to precisely dictate the project's visual intent and design framework.</p><p>While I have a basic coding background, I had never truly participated in a 0-to-1 frontend programming project before. Initially, I tried to take a shortcut — cloning an open-source GitHub repository and asking the AI to comprehend someone else's code before modifying it. I quickly discovered that this approach offers terrible control over the details. Watching the AI generate UIs that were just slightly off from my original design draft was torture for my OCD. Repeatedly debugging just to adjust a margin or align an element ended up burning far more tokens and energy.</p><p>After those headaches, I took a much more cautious approach before genuinely starting the build. I decided to lay a rock-solid foundation to prevent the AI from generating too many \"hallucinations\" that would lead to endless rework. I learned to configure MCP, installed the relevant Skills, and used NotebookLM to generate universal <code>claude.md</code> rules for the project. These rules explicitly defined the AI's standard execution path (Read Page Spec → Access Figma → Plan → Execute → Audit). This maximized the chances that the AI and I were on the exact same page.</p><p>With the environment prepped, I didn't rush the AI into generating specific pages. I had it build the Design System first. We heavily customized an existing public UI library — the process is remarkably similar to a designer building a Design Library; it's fundamentally atomic-to-component development. The AI established foundational Design Tokens based on my specs and strictly aligned the Tailwind configuration.</p><p>Only after this foundation was completely set did we move into the 0-to-1 code generation phase. Thanks to the upfront rule constraints and component limitations, the AI's output during page assembly became highly stable. And throughout this ongoing process, I was Vibe Coding while simultaneously filling gaps and learning actual code architecture.</p>",
      zh: '<p>作为一名 UX 设计师，我的工作流自然是从页面设计开始的（虽然有时候觉得自己的审美可能还比不上一些 AI Skills 产出的效果），但在设计阶段亲自上手，能让我精确把控项目视觉意图和设计框架。</p><p>虽然我有一些基础的代码背景，但此前并没有真正参与过前端从 0 到 1 的编程项目。最初，我也尝试过走捷径——直接克隆一个开源的 Github 仓库，让 AI 先去理解别人的代码再进行修改。但我很快发现，这种方式对细节的把控度极低。看着 AI 生成的 UI 界面与我的原始设计稿总有微妙的出入，对于有强迫症的我来说简直是折磨。为了调整一个间距或是对齐元素而反复 Debug，反而消耗了更多的 Token 和精力。</p><p>有了先前令人头大的经验，这次在真正开始构建网站前，我变得更加谨慎。我决定狠狠打牢基础，以免 AI 产生过多的"幻觉"导致后续反复返工。我学习着首先配置好了 MCP 并安装了相关的 Skills，同时结合 NotebookLM 生成了项目通用的 <code>claude.md</code> 规则。这个规则明确了 AI 的标准执行路径（读取 Page Spec → 访问 Figma → 规划方案 → 执行代码 → 审查校验）。这最大程度上保证了我和 AI 的理解同频。</p><p>环境就绪后，我没有急于让 AI 生成具体页面，而是让它先搭建 Design System。我们在现有的公开 UI 库上进行深度定制。这个过程和设计师搭建 Design Library 非常相似，本质上都是从原子到组件化的开发。AI 会根据我的规范建立底层的 Design Token，并严格对齐 Tailwind 的配置。</p><p>当地基完全打牢之后，才真正进入从 0 到 1 的代码生成阶段。有了前置的规则约束和组件库限制，后续拼装页面时 AI 的输出变得非常稳定。而我也在这个持续推进的过程中，一边 Vibe Coding，一边填补和学习真实的代码知识。</p>',
    },
  },
  ...
  reality: {
    ...
    content: {
      en: '<p>Looking back at the entire run, the time I spent actually typing code was minimal. Most of the time, my role felt more like a strict code reviewer, handing off component breakdowns and responsive logic for the AI to execute. Sometimes, the web animation solutions it provided felt even more natural than what I had originally envisioned.</p><p>But that doesn\'t mean it was easy. Vibe Coding inherently demands extreme project management skills — you are essentially directing several tireless AI assistants simultaneously. In the early stages of the project, as context bounced rapidly across multiple files, my cognitive load spiked. It was incredibly easy to lose my bearings entirely in the chaotic state of tasks.</p><p>To counter this, I brought in skills like <code>planning-with-files</code> to force progress tracking. Concurrently, I dumped all PRDs, debug logs, and system prompts into NotebookLM to build a project-specific knowledge base. Now, when I hit a thorny component interaction issue, I prioritize querying my own knowledge base for a verified solution, rather than blindly asking Claude Code to regenerate the code.</p>',
      zh: '<p>整个项目跑下来，我实际手敲代码的时间很少。大多数时候，我的角色更像是一个严格的代码审核员，将组件拆解和适配逻辑交给 AI 去执行。有时它给出的网页动效方案，甚至比我原本的设想还要自然。</p><p>但这并不意味着轻松。Vibe Coding 本质上是对项目管理能力的极高要求，你相当于同时在指挥几个不知疲倦的 AI 助手。项目初期，由于上下文在多个文件中频繁跳跃，我的认知负荷迅速飙升，很容易在繁杂的任务状态中迷失方向。</p><p>对此我引入了 <code>planning-with-files</code> 这类技能来强制追踪任务进度。同时，我将所有的 PRD、报错复盘以及系统提示词统一整理到 NotebookLM 中，搭建了一个项目专属的知识库。现在遇到棘手的组件联动问题，我会优先向自己的知识库提问，获取经过验证的解决方案，而不是盲目地让 Claude Code 重新生成代码。</p>',
    },
  },
